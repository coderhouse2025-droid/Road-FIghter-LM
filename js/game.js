...
function spawnE(){
  ...
  if(r<0.28){       t=ETYPES[0]; spriteName=t.sp; }
  else if(r<0.50){  t=ETYPES[1]; spriteName=t.sp; }
  else if(r<0.64){  t=ETYPES[2]; spriteName=t.sp; }
  else if(r<0.76){  t=ETYPES[3]; spriteName=t.sp; }
  else if(r<0.92){  // colectivo propio de la zona actual
  ...
  if(isBusImg&&(!sprite||!sprite.complete||sprite.naturalWidth===0)){
    // la foto todavía no cargó (o no existe) — usar un auto como respaldo para no romper el spawn
    t=ETYPES[3]; spriteName=t.sp; label=null;
  }
  const sy=HY+2;
