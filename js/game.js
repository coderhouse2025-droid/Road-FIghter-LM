...
function drawSky(){
  ...
  cld(cx,10,24,8);cld(cx+68,18,16,6);cld(cx+145,6,28,9);
}
function cld(x,y,w,h){
  // nube hecha de bloques (pixel-art), no óvalos suavizados
  const bw=Math.max(2,Math.round(w/6)), bh=Math.max(2,Math.round(h/2));
  const px=Math.round(x-w/2), py=Math.round(y-h/2);
  ctx.fillRect(px+bw,py,bw*4,bh);
  ctx.fillRect(px,py+bh,bw*6,bh);
  ctx.fillRect(px+bw*2,py-bh,bw*3,bh);
}
...
function drawRoad(){
  ...
  dcurb(rlt-0.4,HY,rlb-4,H,9);
  dcurb(rrt,HY,rrb,H,9);
...
function dcurb(x1,y1,x2,y2,bw){
  const n=9,s2=Math.floor(fc*spd*0.28)%2;
  for(let i=0;i<n;i++){
    const t0=i/n,t1=(i+1)/n;
    const ya=y1+(y2-y1)*t0,yb=y1+(y2-y1)*t1;
    const xa=x1+(x2-x1)*t0,xb=x1+(x2-x1)*t1;
    const w0=bw*(t0*.34+.12),w1=bw*(t1*.34+.12);
    ctx.fillStyle=((i+s2)%2===0)?'#a81818':'#f0f0f0';
...
function dtree(x,y,w,h){
  ...
  const ch=Math.round(h*.82);
  // mancha oscura compacta (silueta), como los arbustos del original
  ctx.fillStyle='#0d4a08';
  ctx.fillRect(x,y-ch,w,ch);
  const cw2=Math.max(1,Math.round(w*.62)), ch2=Math.round(ch*.55);
  ctx.fillStyle='#153';
  ctx.fillRect(x+Math.round((w-cw2)/2),y-ch,cw2,ch2);
  // un pequeño resalto claro para que no sea 100% plano
  if(w>=4){
    ctx.fillStyle='#1a6010';
    ctx.fillRect(x+Math.round(w*.15),y-ch+1,Math.max(1,Math.round(w*.3)),Math.max(1,Math.round(ch*.22)));
  }
}
